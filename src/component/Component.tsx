...
// JSX, TSX :
// JSX (JavaScriptXml) / TSX (TypeScriptXml)
...
// 단축 명령어로 컴포넌트를 생성할 수 있음

// 클래스형: rcc(react class component) 단축 명령으로 생성 가능
...
// 함수형: rfc(react functional component) 단축 명령으로 생성 가능
// 함수명 = 파일명과 동일하게 작성됨
...
export default function Component() {
    ...
    // - 부모 요소로 묶을 때 <></>를 이용해서 의미없는 태그를 생성할 수 있음

    // - 반드시 return 바로 뒤에 반환값이 있어야 함

    // JSX(TSX)의 규칙
    // 1. 반드시 하나의 요소를 반환
    // 2. 모든 태그를 받아야 함 (<input/> 과 같은 단일 태그도 마찬가지)
