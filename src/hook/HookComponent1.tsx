...
// - useEffect: 컴포넌트의 생명주기에 따라 특정 작업을 수행할 수 있도록 하는 함수, mount, update, unmount에 해당하느느 생명주기에 따라 실행
...
// - useMeno: 메모리 내에 계산된 값(함수 결과)을 저장하여 함수 재실행 없이 값을 사용할 수 있도록 하는 함수
...
export default function HookComponent1() {
    ...
    // - 개발 실행 환경에서는 mount - unmount가 한 번 실행되고 난 후 다시 mount가 됨
    ...
    // 스코프할 상태 배열에 상태를 지정하면 지정한 상태가 바뀔 때바다 함수가 호출됨
    useEffect(() => {
        // 스코프할 상태 배열에 지정한 상태를 해당 effect에서 변경하면 무한 루프 (개발자 모드라 서버가 실행되면 무조건 안의 내용이 mount(동작)되는데 그러면, update 상태가 되어 배열의 조건([state])을 확인한 후 다시 렌더링이 되고 또 mount 되려 함)
        // setState1(state1 + 1);
        // console.log('state1 상태 변경!');
    }, [state1]);

    // 스코프할 상태 배열에 두 개 이상의 상태를 지정할 수 있음
    // 베열에 포함된 상태 중 하나라도 변경되면 effect가 발생
    useEffect(() => {
