...
// - 상태 관리 패키지 중 과거에는 Redux 패키지가 가장 많이 사용됐지만 현재는 zustand
// - Redux의 고질적 문제점인 복잡한 코드 구조 및 놓은 학습 곡선이 그 이유
// - zustand는 단순한 코드 구조와 학습 곡선이 매우 낮음
// - Redux, MobX 등과 같은 타 패키지에 비해 가벼움 - 빌드 시 패키징 속도가 굉장히

// npm install zustand  ('--'를 붙이면 local PC 자체에 받아짐)

// zustand를 이용한 클로벌 상태 선언 방법
...
// - create 함수의 매개변수로 전달된 콜백함수는 store 객체를 반환해야 함 ( 소괄호 작성 )

// const [address, setAddress] = useState<string>('');

// - typescript에서는 zustand의 store 타입을 create 함수의 제너릭으로 전달해줘야 함

interface Store {
    address: string;
    setAddress: (address: string) => void       // 소괄호 안 매개변수와 그 타입 작성, 화살표 뒤 반환 정보 작성
}

const useStore = create<Store>((set) => ({         // interface Store를 함수명<> 안에 작성
    address: '',
...
}));     // 암기 잘 안 되면 매개변수(address)만 바꿔서 사용해주기
